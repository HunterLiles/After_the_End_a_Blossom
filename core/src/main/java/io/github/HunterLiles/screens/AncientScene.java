...
// Ancient Age level scene
public class AncientScene extends BaseScene {
    // Scene constants
    private static final String SCENE_ID = "ancient_age";
    ...
    // Constructor
    public AncientScene(Main game) {
        super(game);
        playerSpawnPosition = new Vector2(5f, 2f);
    }

    // Set the player spawn position (used when coming from another level)
    ...
    private void createTransitionTriggers() {
        ...
        // Portal to Medieval era
        transitionSystem.createPortalTrigger(
            900, 100, 100, 100,
            "medieval_age",
            5f, 2f,
            "Portal to Medieval Times",
            false
        );
    }
    ...
    private void updateLogic(float delta) {
        // Only update gameplay if the game has started
        if (gameStarted) {
            // Update physics
            ...
            // Update camera without the following player
    ...
    public boolean keyDown(int keycode) {
        // If the game hasn't started, any key press will start it
        if (!gameStarted) {
            gameStarted = true;
